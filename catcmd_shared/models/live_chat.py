...
class CmdVote(ChatCmd):
    ...
    pred_points: Optional[int | Literal["all"]] = None
...
@register
class CmdUsePlatform(ChatCmd):
    command: Literal["useplatform"]
    ...
    def parse_args(cls, tail: list[str]) -> dict:
        if len(tail) < 1:
            raise ValueError("Usage: !useplatform <platform>")
...
class CmdSetLive(ChatCmd):
    ...
    def parse_args(cls, tail: list[str]) -> dict:
        ...
        val = None
        if tail[0].lower() == "true":
            val= True
        elif tail[0].lower() == "false":
            val= False
...
def validate_poll(tail):
    ...
    return {
        "name": tail[0].lower(), "desc": tail[1], "options": options, "close_date": close_date
    }
